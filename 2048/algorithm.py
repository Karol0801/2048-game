...
def create_minimax_tree(depth, board_values):
    ...
    alpha_beta_pruning(1, grid, 0, depth, -math.inf, math.inf, True)


def alpha_beta_pruning(node, grid, parent, depth, alpha, beta, maximizing_player):
    global node_scores
    global child_list
    global node_number
    if depth == 0:
    ...
    if maximizing_player:
        for i in range(4):
            ...
            if move_possible(grid, directions[i]):
                alpha = max(alpha, alpha_beta_pruning(node_number, swipe_grid(grid, directions[i]), node, depth-1, alpha, beta, False))
            if alpha >= beta:
                break
    ...
    # ruch komputera
        ...
        for i in range(4):
            ...
            node_number += 1
            child_list[node].append(node_number)
            beta = min(beta, alpha_beta_pruning(node_number, grid_table[index], node, depth-1, alpha, beta, True))
