...
def get_score(grid):
    ...
    max_value_score = max_value_heuristic(grid) * 40
    smoothness_score = smoothness_heuristic(grid) * 10
    monotonicity_score = monotonicity_heurictic(grid) * 50
    weighted_tiles_score = weighted_tiles_heuristic(grid)

    return smoothness_score + empty_tiles_score + monotonicity_score + max_value_score
